...
!> Variables
    real(kind=8), allocatable :: c1(:),log_c2(:), dc2nc_dc1(:,:)
...
!> Pre-Process
    ...
    allocate(residual(n_p),dc2nc_dc1(n_e,n_p),Delta_c1(n_p),d_log_gamma_d_I(n_nc),out_prod_aq(n_nc_aq,n_nc_aq),&
    log_Jacobian_act_coeffs_aq(n_nc_aq,n_nc_aq),log_Jacobian_act_coeffs(n_nc,n_nc))
    niter=0
    conc_nc(1:n_p)=c1_ig !> chapuza
!> Process
        do
            ...
            call this%compute_c2nc_from_c1_ideal(conc_nc(1:n_p),conc_nc(n_p+1:n_nc))
        ...
        !> We compute Jacobian secondary variable activity-primary concentrations
            call this%compute_dc2nc_dc1_ideal(conc_nc(1:n_p),conc_nc(n_p+1:n_nc),dc2nc_dc1)
