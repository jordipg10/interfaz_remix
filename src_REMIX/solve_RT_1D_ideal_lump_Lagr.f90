...
subroutine solve_RT_1D_ideal_lump_Lagr(this,root)
    use RT_1D_m, only: RT_1D_transient_c, RT_1D_stat_c, RT_1D_c
    use aqueous_chemistry_m, only: aqueous_chemistry_c, &
        water_mixing_iter_EE_eq_kin_ideal_lump, water_mixing_iter_EE_kin_ideal_lump, &
        water_mixing_iter_EI_eq_kin_anal_ideal_opt2_lump, &
        water_mixing_iter_EI_kin_anal_ideal_opt2_lump, mixing_iter_comp_exch_ideal, mixing_iter_comp_ideal_lump, &
        compute_rk_tilde_expl, compute_c_tilde
    implicit none
!> Arguments
    class(RT_1D_c) :: this !> reactive transport object
    ...
    !class(int_array_c), intent(in) :: this%transport%mixing_waters_indices !> matrix that contains indices of target waters that mix with each target water
    !class(int_array_c), intent(in) :: this%transport%mixing_waters_indices_dom !> matrix that contains indices of domain target waters that mix with each target water
...
!> Procedure pointers
    ...
    procedure(water_mixing_iter_EI_kin_anal_ideal_opt2_lump), pointer :: p_solver=>null()
    procedure(compute_rk_tilde_expl), pointer :: compute_rk_tilde=>null()
!> We initialise target waters
    target_waters_old=this%chemistry%target_waters
    target_waters_old_old=target_waters_old
    !this%chemistry%target_waters=target_waters_old
...
!> We select reactive mixing subroutine depending on the nature of the chemical system and the methods to compute Jacobians and integrate in time
    if (this%chemistry%chem_syst%num_kin_reacts>0 .and. this%chemistry%chem_syst%num_eq_reacts>0) then !> equilibrium and kinetic reactions
        if (this%int_method_chem_reacts==1) then !> Euler explicit
            theta=0d0 !> Euler explicit
            p_solver=>water_mixing_iter_EE_eq_kin_ideal_lump
            compute_rk_tilde=>compute_rk_tilde_expl
            mix_ind=1 !> we mix all waters
            !mix_waters_indices=>this%transport%mixing_waters_indices
            ...
            if (this%chemistry%rk_avg_opt==1) then
                ...
                p_solver=>water_mixing_iter_EI_eq_kin_anal_ideal_opt2_lump
            else
            ...
            !    if (this%chemistry%rk_down_opt==1) then
            !        compute_rk_tilde=>compute_rk_tilde_impl_opt1
            !    else if (this%chemistry%rk_down_opt==2) then
            !        compute_rk_tilde=>compute_rk_tilde_impl_opt2
            !    else if (this%chemistry%rk_down_opt==3) then
    ...
    else if (this%chemistry%chem_syst%num_eq_reacts>0) then !> only equilibrium reactions
        p_solver=>mixing_iter_comp_ideal_lump !> only equilibrium reactions
    else !> only kinetic reactions
        if (this%int_method_chem_reacts==1) then !> Euler explicit
            theta=0d0 !> Euler explicit
            p_solver=>water_mixing_iter_EE_kin_ideal_lump
            !compute_rk_tilde=>compute_rk_tilde_expl
            mix_ind=1 !> we mix all waters
        else if (this%chemistry%Jac_opt==1) then !> analytical Jacobian
            if (this%int_method_chem_reacts==2) then !> Euler fully implicit
            ...
            if (this%chemistry%rk_avg_opt==1) then
                ...
                p_solver=>water_mixing_iter_EI_kin_anal_ideal_opt2_lump
            else
                error stop "rk average option not implemented yet"
            end if
            mix_ind=2 !> we mix all waters except one
            ...
            !    else if (this%chemistry%rk_down_opt==3) then
            !       compute_rk_tilde=>compute_rk_tilde_impl_opt3
            !    else if (this%chemistry%rk_down_opt==4) then
    ...
    call this%transport%mixing_ratios_conc%get_can_vec(this%chemistry%CV_params%abs_tol,num_can_vec,ind_can_vec,num_non_can_vec,&
        ind_non_can_vec)
    !> We set old concentrations in target waters (chapuza)
    do i=1,this%chemistry%num_target_waters
        call this%chemistry%target_waters(i)%set_conc_old() !> 
        call this%chemistry%target_waters(i)%set_conc_old_old() !> 
        call this%chemistry%target_waters(i)%solid_chemistry%set_conc_old() !> 
        call this%chemistry%target_waters(i)%solid_chemistry%set_conc_old_old() !> 
        !call initialise_iterative_method()
        !call this%chemistry%target_waters(ind_non_can_vec(i))%update_rk_old()
        !call this%chemistry%target_waters(ind_can_vec(i))%solid_chemistry%update_rk_old()
    end do
    !do i=1,num_non_can_vec
    !    call this%chemistry%target_waters(ind_can_vec(i))%set_conc_old() !>
    !    call this%chemistry%target_waters(ind_can_vec(i))%solid_chemistry%set_conc_old() !>
    !    call this%chemistry%target_waters(ind_can_vec(i))%solid_chemistry%set_conc_old_old() !>
    !end do
        !> Time loop
        do k=1,this%transport%time_discr%Num_time
            ...
            if (k==this%chemistry%chem_out_options%time_steps(kk)) then
               ...
               write(unit,"(20x,*(A15))") (this%chemistry%chem_syst%aq_phase%aq_species(&
            ...
            if (this%transport%tpt_props_heterog%flux_trans%time_ind(k_flux)==k) then
                ...
                call this%transport%stab_params_tpt%compute_stab_params(this%transport%tpt_props_heterog,&
                    this%transport%spatial_discr,Delta_t)
                call this%transport%update_mixing_ratios_Delta_t_homog() !> we update mixing ratios
            ...
            do i=1,this%chemistry%num_target_waters_dom
                call this%chemistry%target_waters(this%chemistry%dom_tar_wat_indices(i))%update_conc_old() !> 
                call this%chemistry%target_waters(this%chemistry%dom_tar_wat_indices(i))%solid_chemistry%update_conc_old() !> 
                call this%chemistry%target_waters(this%chemistry%dom_tar_wat_indices(i))%update_rk_old()
                call this%chemistry%target_waters(this%chemistry%dom_tar_wat_indices(i))%solid_chemistry%update_rk_old()
            end do
            !> Domain target waters loop
            do i=1,this%chemistry%num_target_waters_dom
                n_p=this%chemistry%target_waters(&
                    this%chemistry%dom_tar_wat_indices(&
                    i))%solid_chemistry%reactive_zone%speciation_alg%num_prim_species
                n_nc=this%chemistry%target_waters(this%chemistry%dom_tar_wat_indices(i)&
                    )%solid_chemistry%reactive_zone%speciation_alg%num_var_act_species
                n_nc_aq=this%chemistry%target_waters(this%chemistry%dom_tar_wat_indices(i)&
                    )%solid_chemistry%reactive_zone%speciation_alg%num_aq_var_act_species
                n_nc_aq_2=this%chemistry%target_waters(this%chemistry%dom_tar_wat_indices(i)&
                    )%solid_chemistry%reactive_zone%speciation_alg%num_aq_sec_var_act_species
                !print *, this%chemistry%target_waters(this%chemistry%dom_tar_wat_indices(i))%solid_chemistry%reactive_zone%num_solids
                ...
                !    !call this%chemistry%target_waters(ind_non_can_vec(i))%compute_rk() !> we compute kinetic reaction rates
                !    conc_nc=this%chemistry%target_waters(ind_non_can_vec(i))%get_conc_nc()
                !    !> chapuza
                !    !this%chemistry%target_waters(ind_non_can_vec(i))%Rk_est=0d0
                !    !this%chemistry%target_waters(ind_non_can_vec(i))%solid_chemistry%Rk_est=0d0
                !    continue
                !else
                    if (this%chemistry%target_waters(this%chemistry%dom_tar_wat_indices(&
                    ...
                    !print *, this%chemistry%dom_tar_wat_indices
                    if (this%transport%mixing_waters_indices%cols(ind_non_can_vec(i))%col_1(1)/=this%chemistry%dom_tar_wat_indices(&
                        i)) then
                        print *, this%transport%mixing_waters_indices%cols(ind_non_can_vec(i))%col_1(1),&
                            this%chemistry%dom_tar_wat_indices(i)
                        error stop "Target waters not in the right order"
                    end if
                    !> We solve mixing caused by transport
                    c_tilde=target_waters_old(this%chemistry%dom_tar_wat_indices(i))%get_conc_nc()
                    call compute_c_tilde(target_waters_old(this%transport%mixing_waters_indices%cols(ind_non_can_vec(i))%col_1(&
                        1:this%transport%mixing_waters_indices%cols(ind_non_can_vec(i))%dim-2)),&
                        this%transport%mixing_ratios_conc%cols(ind_non_can_vec(i))%col_1,c_tilde(1:n_nc_aq)) !> we assume mixing ratios are the same for conc and rk (chapuza)
                    !> chapuza
                    !allocate(mix_waters(this%chemistry%num_target_waters_dom-num_can_vec-mix_ind+1))
                    !do j=1,this%chemistry%num_target_waters_dom-mix_ind+1
                    !    if (j==
                    !    mix_waters(j)=this%chemistry%target_waters(this%transport%mixing_waters_indices%cols(i)%col_1(&
                    !    mix_ind+j-1))
                    !mix_waters=this%chemistry%target_waters(this%transport%mixing_waters_indices_dom%cols(ind_non_can_vec(i))%col_1(mix_ind:this%transport%mixing_waters_indices_dom%cols(ind_non_can_vec(i))%dim-2))
                    !call compute_rk_tilde(mix_waters,mixing_ratios_Rk_init%cols(ind_non_can_vec(i))%col_1(mix_ind:),&
                    !    this%transport%mixing_waters_indices_dom%cols(ind_non_can_vec(i))%col_1(this%transport%mixing_waters_indices_dom%cols(ind_non_can_vec(i))%dim-1),&
                    !    this%transport%mixing_waters_indices_dom%cols(ind_non_can_vec(i))%col_1(this%transport%mixing_waters_indices_dom%cols(ind_non_can_vec(i))%dim),theta,Delta_t,rk_tilde)
                    !!> chapuza
                    !this%chemistry%target_waters(this%transport%mixing_waters_indices_dom%cols(ind_non_can_vec(i))%col_1(mix_ind:this%transport%mixing_waters_indices_dom%cols(ind_non_can_vec(i))%dim-2))=&
                    !    mix_waters
                    !call this%chemistry%target_waters(this%chemistry%dom_tar_wat_indices(ind_non_can_vec(i)))%solid_chemistry%modify_mix_ratios_rk(&
                    ...
                    !y=sum(mixing_ratios_Rk%cols(i)%col_1(2:1+this%transport%mixing_waters_indices%cols(i)%col_1(this%transport%mixing_waters_indices%cols(i)%dim-1)))
                    !print *, y*Delta_t
                    !do j=1,n_nc_aq
                    ...
                    !> We solve reactive mixing iteration
                     call p_solver(this%chemistry%target_waters(this%chemistry%dom_tar_wat_indices(i)),target_waters_old_old(&
                        this%chemistry%dom_tar_wat_indices(i))%get_c1(),c_tilde,&
                        Delta_t,theta,conc_nc)
                    !> We compute equilibrium reaction rates from mass balance equation
                    if (this%chemistry%target_waters(this%chemistry%dom_tar_wat_indices(i)&
                        )%solid_chemistry%reactive_zone%speciation_alg%num_eq_reactions>0 .and. &
                        this%chemistry%target_waters(this%chemistry%dom_tar_wat_indices(i))%indices_rk%num_cols>0)&
                            then
                        call this%chemistry%target_waters(this%chemistry%dom_tar_wat_indices(i))%compute_Re_mean_rk_lump(&
                            c_tilde(n_p+1:n_p+n_nc_aq_2),Delta_t,&
                            theta) !> chapuza
                    else if (this%chemistry%target_waters(this%chemistry%dom_tar_wat_indices(i)&
                        )%solid_chemistry%reactive_zone%speciation_alg%num_eq_reactions>0) then
                        call this%chemistry%target_waters(this%chemistry%dom_tar_wat_indices(i))%compute_Re_mean_lump(&
                            c_tilde(n_p+1:n_p+n_nc_aq_2),Delta_t,&
                            theta) !> chapuza
                    end if
                    ! call this%chemistry%target_waters(ind_non_can_vec(i))%compute_r_eq(c_tilde(n_p+1:n_p+n_nc_aq_2),Delta_t,&
                    !     rk_tilde)
                    !> We compute solid chemistry state variables
                    if (associated(this%chemistry%target_waters(this%chemistry%dom_tar_wat_indices(&
                        i))%solid_chemistry%mineral_zone)) &
                        then
                    !> We compute mass volumetric fractions of minerals from mass balance equation
                        call this%chemistry%target_waters(this%chemistry%dom_tar_wat_indices(&
                            i))%solid_chemistry%compute_mass_bal_mins(&
                            Delta_t)
                    !> We compute concentrations of minerals
                        call this%chemistry%target_waters(this%chemistry%dom_tar_wat_indices(&
                            i))%solid_chemistry%compute_conc_minerals_iter_EE(Delta_t)
                    end if
                    !> We compute gas chemistry state variables
                    if (associated(this%chemistry%target_waters(this%chemistry%dom_tar_wat_indices(i))%gas_chemistry)) then
                    !> We compute concentrations of gases
                        call this%chemistry%target_waters(this%chemistry%dom_tar_wat_indices(&
                            i))%gas_chemistry%compute_conc_gases_iter(&
                            Delta_t,&
                            this%chemistry%target_waters(i)%volume,[&
                            this%chemistry%target_waters(this%chemistry%dom_tar_wat_indices(i))%r_eq,&
                            this%chemistry%target_waters(this%chemistry%dom_tar_wat_indices(i))%rk])
                    !> We compute volume of gas
                        call this%chemistry%target_waters(this%chemistry%dom_tar_wat_indices(&
                            i))%gas_chemistry%compute_vol_gas_conc()
                ...
                if ((k==this%chemistry%chem_out_options%time_steps(kk)) .and. (ind_non_can_vec(i)==&
                    this%chemistry%chem_out_options%ind_target_waters(ii))) then
                    !do j=1,n_nc
                        write(unit,"(I10,*(ES15.5))") this%chemistry%chem_out_options%ind_target_waters(ii), (conc_nc(j), j=1,n_nc)
                    !end do
                    if (ii<this%chemistry%chem_out_options%num_target_waters) then
                        ii=ii+1
            ...
            target_waters_old_old=target_waters_old
            target_waters_old=this%chemistry%target_waters
            !> We displace particles
            call this%move_particles(k) !> we displace particles
            !> We introduce new particles
            call this%introduce_particles(k) !> we introduce new particles
        end do
!!> We set the new target waters to the chemistry object
    !this%chemistry%target_waters=this%chemistry%target_waters
    close(unit)
end select
end subroutine