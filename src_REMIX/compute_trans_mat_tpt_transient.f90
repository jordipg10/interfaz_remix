...
!> F*dc/dt=T*c+g
    ...
    type is (mesh_1D_Euler_homog_c) !> homogeneous mesh
        if (mesh%targets_flag.eq.0 .and. this%dimless.eqv..true.) then !> dimensionless, cell centred
            ...
            then !> edge centred, dimensional, homogeneous
            this%trans_mat%sub=this%tpt_props_heterog%dispersion(2:n)/(mesh%Delta_x**2) + &
                this%tpt_props_heterog%flux_int(2:n)/(2*mesh%Delta_x)
            this%trans_mat%super=this%tpt_props_heterog%dispersion(2:n)/(mesh%Delta_x**2) - &
                this%tpt_props_heterog%flux_int(2:n)/(2*mesh%Delta_x)
        end if
