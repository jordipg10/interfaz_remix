!> Computes equilibrium reaction rates from secondary variable activity species concentrations using linear least squares
!! Reaction rates are expressed per unit volume of water
!! We apply lumping to the mixing ratios of reaction rates
subroutine compute_r_eq_lump(this,c2nc_tilde,Delta_t,theta)
...
!> Process
    ...
    this%solid_chemistry%r_eq(1:this%solid_chemistry%reactive_zone%num_minerals_cst_act)=R_eq(1:&
        this%solid_chemistry%reactive_zone%num_minerals_cst_act)/Delta_t !> r_eq=R_eq/Delta_t
    this%solid_chemistry%r_eq(this%solid_chemistry%reactive_zone%num_minerals_cst_act+1:&
    ...
    this%solid_chemistry%r_eq(this%solid_chemistry%reactive_zone%num_minerals+1:this%solid_chemistry%reactive_zone%num_minerals+&
    ...
    if (associated(this%gas_chemistry)) then
        this%gas_chemistry%r_eq(1:this%solid_chemistry%reactive_zone%gas_phase%num_gases_eq_cst_act)=R_eq(&
        ...
        this%gas_chemistry%r_eq(this%solid_chemistry%reactive_zone%gas_phase%num_gases_eq_cst_act+1:&
    ...
    this%r_eq=R_eq(this%solid_chemistry%reactive_zone%num_minerals_cst_act+&
