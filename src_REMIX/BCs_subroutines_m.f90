...
module BCs_subroutines_m
    ...
    contains
         ...
         subroutine Robin_Neumann_homog_BCs(this)
            ...
            integer(kind=4) :: n
            real(kind=8) :: q_cent_1,q_cent_n,q_inf,q_out,D_1,D_2,D_n,c_inf,a,b,alpha,beta,gamma,&
                q_int_1
            ...
            type is (transport_1D_transient_c)
                ...
                D_2=this%tpt_props_heterog%dispersion(2)
                ...
                type is (mesh_1D_Euler_homog_c)
                    if (this%tpt_props_heterog%homog_flag.eqv..true. .and. mesh%scheme<3 .and. mesh%targets_flag==0) then !> CCFD homogeneous
                        ...
                        this%source_term_PDE(1)=this%source_term_PDE(1)+this%BCs%conc_inf*this%bd_mat(1)
                        else if (mesh%targets_flag.eq.1 .AND. this%tpt_props_heterog%homog_flag.eqv..true.) then
                        !> edge centred
                        this%trans_mat%diag(1)=this%trans_mat%diag(1) - q_inf*(q_int_1*mesh%Delta_x+2d0*D_1)/(D_1*mesh%Delta_x) - &
                            2*D_1/(mesh%Delta_x**2)
                        this%trans_mat%super(1)= 2d0*D_2/(mesh%Delta_x**2)
                        this%trans_mat%diag(n)=this%trans_mat%diag(n)-this%trans_mat%sub(n-1)
                        this%bd_mat(1)=q_inf*(q_int_1*mesh%Delta_x+2d0*D_1)/(D_1*mesh%Delta_x)
                        this%source_term_PDE(1)=this%source_term_PDE(1)+this%BCs%conc_inf*this%bd_mat(1)
                    end if
                type is (spatial_discr_rad_c)
                    if (this%dimless.eqv..true.) then
