...
subroutine read_wat_type_CHEPROO(this,n_p_aq,num_cstr,num_gas_zones,model,Jac_opt,unit,niter,CV_flag,gas_chem)
    ...
    !        read(unit,*) this%num_init_wat_types, this%num_bd_wat_types, this%num_rech_wat_types
        ...
        if (index(label,'guess')/=0) then !> 'icon, guess, ctot, constrain'
            ...
            if (gas_flag==1 .and. num_gas_zones==1) then
                call this%set_gas_chemistry(gas_chem) !> chapuza
            else if (gas_flag==1 .and. num_gas_zones==0) then
                error stop "Gas zones not allocated but gas phase found"
            else if (gas_flag==1 .and. num_gas_zones>1) then
