...
!> Arguments
    ...
    real(kind=8), intent(inout) :: c1(:) !> chapuza (dim=n_p)
...
!> Pre-process
    ...
    !> We apply mass action law to compute concentration secondary variable activity species
        log_c2nc_new=matmul(this%solid_chemistry%reactive_zone%speciation_alg%Se_nc_1_star,log_gamma1_old+log10(c1))+&
        this%solid_chemistry%reactive_zone%speciation_alg%logK_star-log_gamma2nc_old !> mass action law
