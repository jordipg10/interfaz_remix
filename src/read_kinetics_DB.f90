    ...
    do
        ...
        if (name=='null') then
            ...
            if (flag.eqv..true. .and. mineral_ind<=this%num_minerals_kin) then
                ...
                do j=1,min_params%num_par_reacts !> we read each parallel reaction
                    read(unit,*) min_params%k(j), min_params%theta(j), min_params%eta(j)
                    do k=1,min_params%num_cat !> we read each catalyst and its power
                        read(unit,*) catalysers(k)%name, min_params%p(j,k)
                    end do
                    do k=1,min_params%num_cat !> we find the index of each catalyst in the chemical system
