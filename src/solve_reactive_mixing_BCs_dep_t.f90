...
subroutine solve_reactive_mixing_BCs_dep_t(this,root,unit,mixing_ratios,mixing_waters_indices,time_discr_tpt,&
    ...
    use aqueous_chemistry_m, only: aqueous_chemistry_c, reactive_zone_c, aq_phase_c, get_c1_aq, get_c1 &
    , water_mixing_iter_EE_eq_kin_ideal, water_mixing_iter_EfI_eq_kin_anal_ideal_opt2, compute_c_tilde, mixing_iter_comp_ideal &
    , mixing_iter_comp_exch_ideal, real_array_c, int_array_c, solid_chemistry_c
...
!> We select reactive mixing subroutine depending on the nature of the chemical system and the methods to compute Jacobians and integrate in time
    if (this%num_reactive_zones>0) then !> there are reactive zones
        ...
        conc_comp_init(:,1)=this%target_waters(this%dom_tar_wat_indices(1))%compute_conc_comp(this%target_waters(&
        this%dom_tar_wat_indices(1))%get_conc_nc())
        conc_comp_init(:,2)=this%target_waters(this%dom_tar_wat_indices(this%num_target_waters_dom))%compute_conc_comp(&
        this%target_waters(this%dom_tar_wat_indices(this%num_target_waters_dom))%get_conc_nc())
        ...
        !> Time loop
            ...
            !> Target waters loop
                do j=1,target_waters_new(this%num_ext_waters+1)%solid_chemistry%reactive_zone%speciation_alg%num_aq_prim_species
                    conc_comp(j,1)=anal_sol_comp(conc_comp_init(j,1),1d0,spatial_discr_tpt%get_Cell_size(),&
                    spatial_discr_tpt%init_point,spatial_discr_tpt%get_Cell_size()*10,time,phi,D,q)
                end do
                call target_waters_new(this%num_ext_waters+1)%compute_c_from_u_bin_syst(conc_comp(1,1),&
                target_waters_new(this%num_ext_waters+1)%solid_chemistry%reactive_zone%chem_syst%eq_reacts(1)%eq_cst) !> chapuza
                ...
                do i=this%num_ext_waters+2,this%num_target_waters-1
                    ...
                    !    continue
                    !else
                        if (this%target_waters(i)%solid_chemistry%reactive_zone%speciation_alg%num_aq_prim_species==&
                        this%target_waters(i)%solid_chemistry%reactive_zone%speciation_alg%num_prim_species) then
                        ...
                        allocate(conc_old(this%target_waters(i)%solid_chemistry%reactive_zone%speciation_alg%num_var_act_species,&
                        mixing_ratios%cols(i-this%num_ext_waters)%dim)) !> chapuza
                        conc_old(:,1)=target_waters_old(i)%get_conc_nc() !> chapuza
                        do j=1,mixing_waters_indices%cols(i-this%num_ext_waters)%dim
                            conc_old(:,1+j)=target_waters_old(mixing_waters_indices%cols(&
                            i-this%num_ext_waters)%col_1(j))%get_conc_nc() !> chapuza
                        end do
                    ...
                    !> We compute equilibrium reaction rates from mass balance equation
                        !call target_waters_new(i)%compute_r_eq_lump(c_tilde(&
                        !    target_waters_new(i)%solid_chemistry%reactive_zone%speciation_alg%num_prim_species+1:&
                        !    target_waters_new(i)%solid_chemistry%reactive_zone%speciation_alg%num_var_act_species),Delta_t)
                    !> Chapuza
                        ...
                        !> We compute concentrations of minerals
                            call target_waters_new(i)%solid_chemistry%compute_conc_minerals_iter_EE(Delta_t)
                        end if
                ...
                do j=1,target_waters_new(this%num_target_waters)%solid_chemistry%reactive_zone%speciation_alg%num_aq_prim_species
                    conc_comp(j,2)=anal_sol_comp(conc_comp_init(j,2),1d0,spatial_discr_tpt%get_Cell_size(),&
                    spatial_discr_tpt%init_point+spatial_discr_tpt%measure,spatial_discr_tpt%get_Cell_size()*10,time,phi,D,q)
                end do
                call target_waters_new(this%num_target_waters)%compute_c_from_u_bin_syst(conc_comp(1,2),target_waters_new(&
                    this%num_target_waters)%solid_chemistry%reactive_zone%chem_syst%eq_reacts(1)%eq_cst)
