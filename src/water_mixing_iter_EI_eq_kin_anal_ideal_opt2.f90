...
!> Variables
    ...
    real(kind=8), allocatable :: u_tilde(:) !> component concenratuiobs after mixing
...
!> Pre-process
    ...
    !> We compute complementary reaction part of components after mixing (llamalo de otra forma)
        u_rk_tilde=Delta_t*matmul(this%solid_chemistry%reactive_zone%speciation_alg%comp_mat,rk_tilde)
!> Process
    !> Initial guess aquoeus primary concentrations
        call this%initialise_conc_nc_iterative_method_ideal(mu,conc_nc) !> we set initial guess for variabler activity concentrations
        ...
        do
            ...
            !> We apply Newton method to compute aqueous concentrations
                call this%Newton_EI_rk_eq_kin_aq_anal_ideal_opt2(u_tilde,u_rk_tilde,mix_ratio_Rk,Delta_t_bis,theta,conc_nc,niter,&
                    CV_flag)
            !> We check convergence
                if (CV_flag.eqv..false.) then !> no CV
                    ...
                    !call this%initialise_c1_aq_iterative_method(mu) !> we re-initialise Newton method
                    call this%initialise_conc_nc_iterative_method_ideal(mu,conc_nc) !> we reset initial guess for variabler activity concentrations
                    call this%set_act_aq_species() !> we set activity of aqueous species
                else 
                    k=k+1
                    exit
