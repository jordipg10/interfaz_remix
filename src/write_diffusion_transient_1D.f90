...
subroutine write_diffusion_transient_1D(this,Time_out,output)
    ...
    write(file_out,"('diffusion_transient_dim',I1,'_n',I2,'.out')") this%spatial_discr%dim, n
    ...
    if (this%dimless.eqv..true.) then
        ...
        if (this%BCs%labels(1) == 1 .and. this%BCs%labels(2) == 1) then
            write(1,"(2x,'Boundary conditions:',10x,'Dirichlet',/)")
        else if (this%BCs%labels(1) == 2 .and. this%BCs%labels(2) == 2) then
        ...
        if (this%diff_props_heterog%homog_flag.eqv..true.) then
            write(1,"(2x,'Diffusion properties:',10x,'Homogeneous',/)")
        else
            write(1,"(2x,'Diffusion properties:',10x,'Heterogeneous',/)")
        end if
        !write(1,"(2x,'Properties:'/)")
        write(1,"(10x,'Porosity:',ES15.5,10x,'Diffusion at cell centres:',ES15.5/)") this%diff_props_heterog%porosity(1), &
            this%diff_props_heterog%diff_cent(1)
        ...
        if (this%sol_method.eq.2) then
            call this%compute_A_mat_ODE_1D(A_mat)
        ...
        if (this%sol_method.eq.1) then
            ...
            do i=1,n
                write(1,"(2x,I4,*(F15.5),F15.5)") i,(output(i,k), k=1,Num_output), this%conc_obj%conc(i)
            end do
            ...
            do k=1,Num_output
                ...
                !bd_flux(k)=(this%BCs%conc_inf*this%diff_props_heterog%dispersion(1)/this%char_params%char_length)*&
                !    sum_flux*4d0/(&
                !    Delta_r(n)**2)
            end do
