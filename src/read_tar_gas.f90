    ...
    type(gas_chemistry_c), target :: gas_species_chem !> default gas chemistry object
    ...
    !    call gas_species_chem%set_reactive_zone(this%reactive_zones(this%num_reactive_zones)) !> we set default reactive zone in default gas chemistry object
    !    call gas_species_chem%set_mineral_zone(this%mineral_zones(1)) !> we set default mineral zone in default gas chemistry object
    !end if
    ...
    do
        ...
        if (label=='end') then
            ...
            do
                ...
                if (ind_bar>0) then !> we have a range of target gases
                    ...
                    if (first<1 .or. first>last .or. last>num_tar_gas) then
                        ...
                        do i=first,last
                            call this%target_gases(i)%copy_gas_chemistry(this%gas_zones(igzone)) !> we set the initial gas type
                            call this%target_gases(i)%set_id(igzone) !> the id of the gas chemistry object is the index of the gas zone
                            ...
                            call this%target_gases(i)%reactive_zone%allocate_ind_var_act_species() !> chapuza
                            ...
                            call this%target_gases(i)%allocate_reaction_rates_gas_species_chem() !> we allocate reaction rates in gas chemistry object
                        end do
                    end if
                    ...
                    if (ind_tar_gas<1 .or. ind_tar_gas>num_tar_gas) then
                        ...
                        call this%target_gases(ind_tar_gas)%copy_gas_chemistry(this%gas_zones(igzone)) !> we set the initial gas type
                        call this%target_gases(ind_tar_gas)%set_id(igzone) !> the id of the gas chemistry object is the index of the gas zone
                        ...
                        call this%target_gases(ind_tar_gas)%reactive_zone%allocate_ind_var_act_species() !> chapuza
            ...
            !         read(unit,*) tar_wat_ind, wtype, public, igzone, igzn, flag_wat_type
            ...
            !             igzone,igzn,aux_igzone,aux_igzn,gas_species_chem)
                ...
                !            call this%waters(tar_wat_ind)%set_gas_chemistry(this%target_gases(tar_wat_ind))
                !            call gas_species_chem%set_reactive_zone(this%reactive_zones(igzn))
                !        else
                ...
                !            call gas_species_chem%set_reactive_zone(react_zone)
                !        end if
                !        call gas_species_chem%set_mineral_zone(min_zone)
                !        call this%waters(tar_wat_ind)%set_gas_chemistry(gas_species_chem)
                !    end if
                ...
                !        call this%waters(tar_wat_ind)%gas_chemistry%reactive_zone%set_speciation_alg_dimensions(flag_comp)
                !        call this%waters(tar_wat_ind)%gas_chemistry%reactive_zone%set_ind_eq_reacts() !> chapuza
                !        call this%waters(tar_wat_ind)%gas_chemistry%reactive_zone%set_stoich_mat_react_zone() !> chapuza
                !        call this%waters(tar_wat_ind)%gas_chemistry%reactive_zone%set_ind_gases_stoich_mat() !> chapuza
                !        call this%waters(tar_wat_ind)%gas_chemistry%reactive_zone%set_ind_mins_stoich_mat() !> chapuza
                !        call this%waters(tar_wat_ind)%set_ind_species()
                !        call this%waters(tar_wat_ind)%gas_chemistry%reactive_zone%compute_speciation_alg_arrays(&
                !            flag_Se,swap)
                ...
                !            !     this%waters(tar_wat_ind)%gas_chemistry%reactive_zone%speciation_alg%num_prim_species)
                ...
                !    print *, this%waters(tar_wat_ind)%ind_var_act_species
                !!> Chapuza
                !    !if (associated(this%waters(tar_wat_ind)%gas_chemistry%mineral_zone)) then
                !        if (this%waters(tar_wat_ind)%gas_chemistry%mineral_zone%num_minerals_kin<&
                !            this%waters(tar_wat_ind)%gas_chemistry%reactive_zone%chem_syst%num_minerals_kin) then
                !            call this%waters(tar_wat_ind)%gas_chemistry%reactive_zone%compute_U_SkT_prod(&
                !                this%waters(tar_wat_ind)%gas_chemistry%reactive_zone%chem_syst%num_lin_kin_reacts+&
                !                this%waters(tar_wat_ind)%gas_chemistry%mineral_zone%ind_min_chem_syst(1:&
                !                this%waters(tar_wat_ind)%gas_chemistry%mineral_zone%num_minerals_kin))
                !        else
                !            call this%waters(tar_wat_ind)%gas_chemistry%reactive_zone%compute_U_SkT_prod()
                !        end if
                !    !end if
    ...
    !         else
    !             do i=1,this%num_target_waters
    !                 read(unit,*) tar_wat_ind, wtype, public, igzone, igzn
    !                 if (tar_wat_ind<1 .or. tar_wat_ind>this%num_target_waters) then
    !                     error stop "Target water index out of bounds"
    !                 else if (wtype<1 .or. wtype>nwtype) then
    !                     error stop "Water type index out of bounds"
    ...
    !                     wtype, public,igzone,igzn,aux_igzone,aux_igzn,gas_species_chem)
    !             !    !aux_igzone=igzone
    !             !    this%waters(tar_wat_ind)=water_types(wtype)
    !             !    ! if (counter_swap==0) then
    !             !    !     call this%waters(tar_wat_ind)%set_aq_phase(this%chem_syst%aq_phase)
    ...
    !             !    !         call this%waters(j)%gas_chemistry%reactive_zone%set_speciation_alg_dimensions(&
    !             !    !             flag_comp)
    ...
    !             !    !             this%waters(j)%gas_chemistry%reactive_zone%speciation_alg%num_prim_species)=swap(1) !> chapuza
    ...
    !             !    !     this%waters(tar_wat_ind)%ind_sec_species(swap(2)-&
    !             !    !         this%waters(tar_wat_ind)%gas_chemistry%reactive_zone%speciation_alg%num_prim_species)=swap(1) !> chapuza
    !             !    ! end if
    !             !    if (this%waters(tar_wat_ind)%gas_chemistry%mineral_zone%num_minerals_kin<&
    !             !        this%waters(tar_wat_ind)%gas_chemistry%reactive_zone%chem_syst%num_minerals_kin) then
    !             !        call this%waters(tar_wat_ind)%gas_chemistry%reactive_zone%compute_U_SkT_prod(&
    ...
    this%target_gases_init=this%target_gases !> we assign the target gases to the initial target gases
end subroutine