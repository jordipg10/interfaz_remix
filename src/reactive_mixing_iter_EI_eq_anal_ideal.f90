!> \file reactive_mixing_iter_EI_eq_anal_ideal.f90
!> \brief Reactive mixing iteration with Euler-implicit equilibrium reactions for ideal systems.
!> \details This subroutine performs a single reactive mixing iteration of the WMA (Water Mixing Approach)
!> for equilibrium-only systems (no kinetic reactions). It solves for the variable activity species
...
!> - **Equilibrium-only** speciation (no kinetic source terms)
!>
...
!>
!> \sa Newton_EI_eq_ideal, initialise_iterative_method

subroutine reactive_mixing_iter_EI_eq_anal_ideal(this,c1_old,c_hat,mix_ratio_r_old,mix_ratio_r_new,Delta_t,theta,conc_nc,conc_comp,c1_downstream)
...
!> -----------------------------------------------------------------------
!> \section rmix_eq_preproc Pre-processing: extract dimensions, cache matrices, compute initial guess
!> -----------------------------------------------------------------------
...
!> -----------------------------------------------------------------------
!> \section rmix_eq_process Process: Newton iteration with adaptive time-stepping
!> -----------------------------------------------------------------------
    ...
    call this%set_conc_var_act_species(conc_nc)                               !< Update object concentrations with initial guess
    call this%set_act_aq_species()                                           !< Set activity of aqueous species from initial guess concentrations
    ...
    do                                                                       !< Outer loop: accumulate \f$ 2^{k_{\text{div}}} \f$ converged sub-steps
        do                                                                   !< Inner loop: retry Newton with varying \f$ \mu \f$ or halved \f$ \Delta t \f$
            !> Solve the nonlinear equilibrium system via Newton-Raphson
            call this%Newton_EI_eq_ideal(u_hat,conc_nc,niter,CV_flag)        !< Call Newton solver for equilibrium speciation
            ...
            if (mu<1d0) then                                                 !< Have we exhausted all \f$ \mu \f$ values?
                ...
                if (k_div>k_div_max) then                                     !< Exceeded maximum allowed divisions?
                    if (.not. c_hat_fallback_tried) then                       !< c_hat fallback not yet attempted?
                        ...
                        call this%set_conc_var_act_species(conc_nc)            !< Update object concentrations
                        call this%set_act_aq_species()                         !< Reset activities
                    ...
                    error stop "Too many time step divisions in reactive_mixing_iter_EI_eq_anal_ideal. &
            ...
            call this%set_conc_var_act_species(conc_nc)                       !< Update object concentrations with retried initial guess
            call this%set_act_aq_species()                                   !< Reset activity of aqueous species from updated concentrations
...
!> -----------------------------------------------------------------------
!> \section rmix_eq_postproc Post-processing: compute component concentrations and deallocate
!> -----------------------------------------------------------------------
    call this%set_conc_var_act_species(conc_nc)                              !< Set variable activity species concentrations in the chemistry object
    call this%set_act_aq_species()                                            !< Update aqueous species activities based on converged concentrations
    call this%compute_pH()                                                   !< Compute pH from the converged aqueous species concentrations
...
end subroutine                                                               !< End reactive_mixing_iter_EI_eq_anal_ideal