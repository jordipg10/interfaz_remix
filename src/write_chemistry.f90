        ...
        if (num_dom>0) then
            ...
            do i=1,this%chem_syst%aq_phase%num_species
             ...
             do i=1,this%reactive_zones(l)%num_minerals
                 !print *, this%target_solids_init(&
                 !    dom_indices(1))%mineral_zone%num_minerals_kin
                  write(unit,"(10x,*(ES15.5))") (this%target_solids_init(j)%vol_fracts(this%target_solids_init(&
                      j)%mineral_zone%num_minerals_kin+i), j=1,size(dom_indices))
             end do
             write(unit,"(/,10x,'Volumetric fractions of minerals:'/)")
             do i=1,this%reactive_zones(l)%num_minerals
                 write(unit,"(10x,*(ES15.5))") (this%target_solids(j)%vol_fracts(this%target_solids(&
                     j)%mineral_zone%num_minerals_kin+i), j=1,size(dom_indices))
             end do
             write(unit,"(/,10x,'Initial concentration of minerals:'/)")
             do i=1,this%reactive_zones(l)%num_minerals
                 write(unit,"(10x,*(ES15.5))") (this%target_solids_init(j)%concentrations(this%target_solids_init(&
                     j)%mineral_zone%num_minerals_kin+i), j=1,size(dom_indices))
             end do
             write(unit,"(/,10x,'Concentration of minerals:'/)")
             do i=1,this%reactive_zones(l)%num_minerals
                 write(unit,"(10x,*(ES15.5))") (this%target_solids(j)%concentrations(this%target_solids(&
                     j)%mineral_zone%num_minerals_kin+i), j=1,size(dom_indices))
             end do
            write(unit,"(/,10x,'Initial concentration of surface complexes:'/)")
            do i=1,this%reactive_zones(l)%cat_exch_zone%num_surf_compl
                write(unit,"(10x,*(ES15.5))") (this%target_solids_init(j)%concentrations(&
                    this%target_solids_init(j)%mineral_zone%num_minerals+i), j=1,size(dom_indices))
            end do
            write(unit,"(/,10x,'Concentration of surface complexes:'/)")
            do i=1,this%reactive_zones(l)%cat_exch_zone%num_surf_compl
                write(unit,"(10x,*(ES15.5))") (this%target_solids(j)%concentrations(&
                    this%target_solids(j)%mineral_zone%num_minerals+i), j=1,size(dom_indices))
            end do
            write(unit,"(/,10x,'Initial activity of surface complexes:'/)")
            do i=1,this%reactive_zones(l)%cat_exch_zone%num_surf_compl
                write(unit,"(10x,*(ES15.5))") (this%target_solids_init(j)%activities(&
                this%target_solids_init(j)%mineral_zone%num_minerals+i), j=1,size(dom_indices))
            end do
            write(unit,"(/,10x,'Activity of surface complexes:'/)")
            do i=1,this%reactive_zones(l)%cat_exch_zone%num_surf_compl
                write(unit,"(10x,*(ES15.5))") (this%target_solids(j)%activities(&
                    this%target_solids(j)%mineral_zone%num_minerals+i), j=1,size(dom_indices))
            end do
