subroutine solve_diff_EE_Delta_t_heterog(this,Time_out,output)
    !> Solves 1D transient diffusion PDE with heterogeneous time step using Lagr explicit method
    ...
    use time_discr_m, only: time_discr_heterog_c
    implicit none
    ...
    !class is (diffusion_1D_transient_c)
        ...
        !> Explicit Euler
            ...
            !> Spatial discretisation
                call this%compute_b_vec_1D(0d0,conc_old,conc_new,k)
                if (abs(Time-Time_out(icol))<epsilon) then
