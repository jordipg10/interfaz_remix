...
subroutine set_stoich_mat_sol(this)
    ...
    !> We set surface complexes
        if (this%cat_exch_zone%num_surf_compl>0) then
            ...
            do
                if (this%cat_exch_zone%surf_compl(i)%name==this%species(this%eq_reacts(j)%species_ind(k))%name) then
    ...
    !> We set minerals
        if (this%num_minerals_eq>0) then
            ...
            do
                if (this%minerals(this%num_minerals_kin+i)%mineral%name==this%species(this%eq_reacts(j)%species_ind(k))%name) then
                    this%stoich_mat_sol(j,this%cat_exch_zone%num_surf_compl+this%num_minerals_kin+i)=this%eq_reacts(j)%stoichiometry(k)
                    if (j<this%speciation_alg%num_eq_reactions) then
                        j=j+1 
                        k=1
        ...
        if (this%num_minerals_kin>0) then
            ...
            do
                if (this%minerals(i)%mineral%name==this%species(this%min_kin_reacts(j)%species_ind(k))%name) then
                    this%stoich_mat_sol(this%speciation_alg%num_eq_reactions+this%num_lin_kin_reacts+j,this%cat_exch_zone%num_surf_compl+i)=&
