...
subroutine read_PHREEQC_DB_opc2(this,filename)
    ...
    use kin_reaction_m, only: kin_reaction_c, kin_reaction_poly_c
    ...
    integer(kind=4) :: aq_counter,min_counter,gas_species_counter,eq_ind,valence,line_count,i,j,k,l,num_surf_compl,num_exch_sp,num_mins,&
    ...
    type(gas_species_c), allocatable :: gases(:)
    ...
    open(unit=61,file=filename,status='old',action='read')
    line_count=1
    ...
    gas_species_counter=0 !> gas counter
    do
        read(61,"(A100)",iostat=end_of_file) str_block
        ...
        if (str_block(1:len_trim(str_block))=='SOLUTION_MASTER_SPECIES') then
            ...
            do
                ...
                !>    backspace(5)
                !>    read(61,*) str, aq_species%params_act_coeff%a_TJ, aq_species%params_act_coeff%b_TJ
                else if (index(str_trim,this%aq_phase%aq_species(aq_counter)%name)/=0) then 
                    backspace(61)
                    read(61,*) elem%name, aq_species%name, alk_contrib, gfw_form, gfw_elem
            ...
            do
                ...
                if (index(str_trim,'#')==1 .or. index(str_trim,'#')==2) then
                    ...
                    phase=>gas_phase
                    if (gas_species_counter<this%gas_phase%num_species) then
                        gas_species_counter=gas_species_counter+1
                    end if
                    exit
                end if
                do !while (str_trim(1:1)/='')
                    ...
                    if (index(str_trim,'#')==1 .or. index(str_trim,'#')==2) then
                        ...
                        index(str_trim,this%gas_phase%gases(gas_species_counter)%name)/=0) then
                        if (eq_react%num_species>1) then
                            !call this%append_reaction(eq_react)
                            call eq_react%deallocate_reaction()
                        end if
                        ...
                        !> call eq_react%allocate_coeffs_logK_T()
                        !backspace(5)
                        !read(61,*) str, (eq_react%coeffs_logK_T(j), j=1,6)
                    else if (index(str_trim,'Vm')==3) then
                        backspace(61)
                        read(61,*) str, this%minerals(min_counter)%mineral%mol_vol
                    else if (index(str_trim,'T_c')==3) then
                        backspace(61)
                        read(61,*) str, this%gas_phase%gases(gas_species_counter)%crit_temp
                    else if (index(str_trim,'P_c')==3) then
                        backspace(61)
                        read(61,*) str, this%gas_phase%gases(gas_species_counter)%crit_press
                    else if (index(str_trim,'Omega')==3) then
                        backspace(61)
                        read(61,*) str, this%gas_phase%gases(gas_species_counter)%acentric_fact
                    else if (str_trim(1:1)/='') then
                        backspace(61)
                        exit
                    else
    ...
    !>                    backspace(5)
    !>                    call reactive_zone%allocate_ind_non_flow_species(num_mins)
    !>                    call solid_chem_init%set_reactive_zone(reactive_zone)
    !>                    call solid_chem_init%allocate_concentrations()
    !>                    do i=1,num_mins
    !>                        backspace(5)
    !>                        read(5,*) reactive_zone%ind_non_flow_species(i)%name, target_SI, solid_chem_init%concentrations(i)
    !>                        reactive_zone%ind_non_flow_species(i)%cst_act_flag=.true.
    !>                        backspace(5)
    !>                    end do
    !>                end if
    ...
    !!print *, this%target_solids(1)%reactive_zone%num_non_flow_species
    !!print *, this%target_solids(1)%reactive_zone%ind_non_flow_species(1)%name
