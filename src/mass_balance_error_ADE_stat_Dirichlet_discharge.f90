function mass_balance_error_ADE_stat_Dirichlet_discharge(this) result(mass_bal_err)
    ...
    type is (mesh_1D_Euler_homog_c) 
        ...
        inf_term = 2d0*this%tpt_props_heterog%disp_int(1)*(this%BCs%conc_inf-this%conc(1))/mesh%Delta_x+&
            this%BCs%flux_inf*this%BCs%conc_inf
        out_term = 2d0*this%tpt_props_heterog%disp_int(n)*(this%BCs%conc_out-this%conc(n))/mesh%Delta_x-&
