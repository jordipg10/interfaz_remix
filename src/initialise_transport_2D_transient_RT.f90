!> \file initialise_transport_2D_transient_RT.f90
!> \brief Initialize 1D transient transport object for reactive transport (RT) simulation
!>
!> \details
!> This subroutine initializes a 2D transient transport object specifically configured for 
...
subroutine initialise_transport_2D_transient_RT(this,path,root,mesh_type)
    ...
    use BCs_m, only: BCs_2D_c !> Import boundary conditions type
    ...
    use transport_stab_params_m, only: stab_params_tpt_2D_c !> Import stability parameters type for transport
    ...
    use transport_transient_m, only: transport_2D_transient_c !> Import 2D transient transport class
    use transport_properties_heterog_m, only: tpt_props_heterog_2D_c !> Import heterogeneous transport properties class
    ...
    class(transport_2D_transient_c) :: this !> Transport 2D transient object to be initialized (contains mesh, time discretization, BCs, properties)
    ...
    type(tpt_props_heterog_2D_c) :: my_props_tpt !> Heterogeneous transport properties object (porosity, dispersion, flux)
    ...
    type(BCs_2D_c) :: my_BCs !> Boundary conditions object (contains BC types, values, and flux data)
    type(stab_params_tpt_2D_c) :: my_stab_params_tpt !> Transport stability parameters object (Courant number, Peclet number, critical time step)
...
!> Mesh (chapuza)
    ...
    if (mesh_type.eq.1) then !> Check if mesh type is 1 (1D homogeneous)
        allocate(mesh_2D_Euler_homog_c :: my_mesh) !> Allocate pointer to 1D homogeneous Eulerian mesh class
...
!> Boundary conditions
    ...
    call this%set_BCs_2D_trans(my_BCs) !> Store boundary conditions object in transport object
...
!> Transport properties configuration
    call my_props_tpt%allocate_tpt_props(this%spatial_discr%Num_targets - this%spatial_discr%targets_flag) !> Allocate base 1D transport property arrays (flux, porosity, source term)
    call my_props_tpt%allocate_disp_tensor( &
        this%spatial_discr%get_num_cells(1), &
        this%spatial_discr%get_num_cells(2)) !> Allocate dispersion tensor with correct rectangular shape (Nx+1, Ny+1, 3)
    ...
    else if (my_props_tpt%source_term_order>0) then !> Source term is polynomial (order > 0): f(x) = ∑ᵗₕᵢ ᵃᵢˣⁱ
    !> Temporary workaround solution (should be refactored)
        ...
        call my_props_tpt%compute_flux_nonlin(flux_coeffs,this%spatial_discr) !> Compute nonlinear (polynomial) flux field from coefficients q(x) = ∑ᵗₕᵢ ᵃᵢˣⁱ [L/T]
    ...
    call my_props_tpt%compute_disp_tensor_2D() !> Compute dispersion at cell interfaces from cell-centered values using numerical scheme [L²/T]
    call this%set_tpt_props_heterog_obj_2D(my_props_tpt) !> Assign transport properties object to transport%tpt_props_heterog member [-]
!****************************************************************************************************************************************************
!> Stability parameters computation
    call my_stab_params_tpt%compute_stab_params_tpt_2D(this%tpt_props_heterog,my_mesh,my_time_discr%get_Delta_t()) !> Compute Courant number Co = qΔt/Δx and Peclet number Pe = qΔx/D for stability analysis [-]
    call this%set_stab_params_tpt_2D(my_stab_params_tpt) !> Assign stability parameters object to transport%stab_params_tpt member [-]
!**************************************************************************************************************************************************
