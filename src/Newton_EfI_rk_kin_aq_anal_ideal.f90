...
!> Process
    ...
    !> Newton loop
        ...
        !> Newton residual
            ...
            fk=conc_nc-c_tilde-Rk_tilde-Delta_t*mix_ratio_Rk*matmul(this%solid_chemistry%reactive_zone%U_SkT_prod,rk)
        !> Check convergence
            if (inf_norm_vec_real(fk)<this%solid_chemistry%reactive_zone%CV_params%abs_tol) then !> CV reached
                CV_flag=.true.
                exit
            else
                call this%compute_dfk_dc_aq_EfI(drk_dc,Delta_t,1d0,mix_ratio_Rk,dfk_dc) !> computes Jacobian of Newton resiudal
...
!> Post-process
        deallocate(fk,dfk_dc,Delta_c,drk_dc,rk,c_rk_tilde)
end subroutine