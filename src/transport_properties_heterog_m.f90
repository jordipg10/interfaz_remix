        ...
        subroutine read_tpt_props_heterog(this,root,spatial_discr)
            ...
            !     error stop "Flux cannot be constant"
                ...
                if (trans_flag.eqv..true.) then
                    ...
                    open(unit=5,file=root//'_flow_inf.dat',status='old',action='read')
                    call this%flux_trans%read_time_series(5) !> we assume homogeneous flux
                    close(5)
            ...
            if (flag.eqv..true.) then
                ...
                if (spatial_discr%scheme == 2 .and. spatial_discr%targets_flag == 0) then !> cell centred, proposed by Petchamé & Carrera (2024)
