!> \file flow_transient_m.f90
!> \brief 1D transient flow module for chemical transport simulations.
!> \details Defines the flow_transient_c class, its properties, and procedures for solving, initializing, and managing transient flow problems in 1D domains.
module flow_transient_m
    ...
    save
    private
    !> \class flow_transient_c
    !> \brief 1D transient flow subclass for chemical transport.
    !> \details Extends PDE_1D_transient_c and contains hydraulic head, initial head, heterogeneous flow properties, stability and characteristic parameters, and procedures for solving and managing transient flow.
    type, public, extends(PDE_1D_transient_c) :: flow_transient_c
        !> \var head Hydraulic head
        real(kind=8), allocatable :: head(:)
        !> \var head_init Initial hydraulic head
        real(kind=8), allocatable :: head_init(:)
        !> \var flow_props_heterog Heterogeneous flow properties
        type(flow_props_heterog_conf_c) :: flow_props_heterog
        !> \var stab_params_flow Stability parameters
        type(stab_params_flow_c) :: stab_params_flow
        !> \var char_params_flow Characteristic parameters
        type(char_params_flow_c) :: char_params_flow
    contains
        !> \brief Allocates hydraulic head arrays
        procedure :: allocate_head
        !> \brief Sets hydraulic head
        procedure :: set_head
        !> \brief Sets initial hydraulic head
        procedure :: set_head_init
        !> \brief Computes F matrix for PDE
        procedure :: compute_F_mat_PDE=>compute_F_mat_flow
        !> \brief Computes transition matrix for PDE
        procedure :: compute_trans_mat_PDE=>compute_trans_mat_flow_transient
        !> \brief Sets stability parameters for flow
        procedure :: set_stab_params_flow
        !> \brief Sets characteristic parameters for flow
        procedure :: set_char_params_flow
        !> \brief Initializes PDE for flow
        procedure :: initialise_PDE=>initialise_flow_transient
        !> \brief Computes source term for flow
        procedure :: compute_source_term_PDE=>compute_source_term_flow
        !> \brief Computes recharge matrix for flow
        procedure :: compute_rech_mat_PDE=>compute_rech_mat_flow
        !> \brief Writes PDE solution for flow
        procedure :: write_PDE=>write_flow_transient
        !> \brief Sets heterogeneous flow properties
        procedure :: set_flow_props_heterog
        !> \brief Solves PDE for transient flow
        procedure :: solve_PDE=>solve_flow_transient
        !> \brief Solves flow with explicit Euler time stepping
        procedure :: solve_flow_EE_Delta_t_homog
        !> \brief Solves flow with implicit Euler time stepping
        procedure :: solve_flow_EI_Delta_t_homog
        procedure :: compute_mixing_ratios_Delta_t_homog=>compute_mixing_ratios_Delta_t_homog_flow
    end type flow_transient_c
    ...
    interface
        ...
        subroutine initialise_flow_transient(this,path,root,mesh_type)
            import flow_transient_c
            implicit none
            class(flow_transient_c) :: this
        ...
        subroutine solve_flow_transient(this)
        import flow_transient_c
        implicit none
        ...
        subroutine write_flow_transient(this)
            import flow_transient_c
            implicit none
    ...
    !> \brief Sets the hydraulic head array
    !> \param this Flow object
    !> \param head Hydraulic head array to set
    ...
    !> \brief Sets the initial hydraulic head array
    !> \param this Flow object
    !> \param head Initial hydraulic head array to set
    ...
    end subroutine set_head_init
    
    !> \brief Sets the stability parameters for flow
    !> \param this Flow object
    !> \param stab_params_flow Stability parameters to set
    ...
    end subroutine set_stab_params_flow
        
    !> \brief Sets the heterogeneous flow properties
    !> \param this Flow object
    !> \param flow_props_heterog Heterogeneous flow properties to set
    ...
    end subroutine set_flow_props_heterog
    ...
    !> \brief Allocates the hydraulic head and initial head arrays
    !> \param this Flow object
    ...
    end subroutine allocate_head
        
    !> \brief Sets the characteristic parameters for flow
    !> \param this Flow object
    !> \param char_params_flow Characteristic parameters to set
    ...
    end subroutine set_char_params_flow

    subroutine compute_mixing_ratios_Delta_t_homog_flow(this)
        implicit none
        class(flow_transient_c) :: this
        ...
        !     if (conc_new(i)>0d0) then
        !         mixing_ratios(1,i)=conc_old(i)/conc_new(i)
        !     else
        !         mixing_ratios(1,i)=1d0 !> If new concentration is zero, set mixing ratio to 1 to avoid division by zero (no change in concentration)
        !     end if
        !     mixing_ratios(2,i)=Delta_t/Delta_x !> Time step to spatial step ratio for stability analysis
        ! end do
        
    end subroutine compute_mixing_ratios_Delta_t_homog_flow
