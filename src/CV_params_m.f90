...
module CV_params_m
    ...
    type, public :: CV_params_s !< Public structure holding all convergence parameters for iterative methods
        ...
        !> \var log_abs_tol Logarithmic absolute tolerance for Picard residuals
        !> Convergence criterion: log₁₀(||f(x)||) < log_abs_tol
        !> Default: -14 [-] - log₁₀ of acceptable mass balance error
        !> Physical meaning: log₁₀ threshold for residuals (dimensionless exponent, not a concentration)
        real(kind=8) :: log_abs_tol=-14 !< [-] log₁₀ absolute tolerance for Picard residuals
        !> \var log_rel_tol Logarithmic relative tolerance for Newton residuals
        !> Convergence criterion: |log₁₀(Δc)/log₁₀(c)| < 10^log_rel_tol for concentrations
        !> Default: -12 [-] - stored as the log₁₀ of the dimensionless tolerance
        ...
        real(kind=8) :: log_rel_tol=-12 !< [-] log₁₀ relative tolerance for concentration changes
        
        !> \var rel_tol Relative tolerance for convergence
        !> Convergence criterion: ||Δx||/||x|| < rel_tol where Δx is Newton update
        !> Default: 1×10⁻¹² [-] - strict relative convergence
        !> Physical meaning: fractional change in solution vector between iterations
        real(kind=8) :: rel_tol=1d-12 !< [-] relative tolerance for solution updates (dimensionless ratio)
        ...
        !> Default: 10 [#] - allows Δt to be reduced to Δt/2^10 = Δt/1024 for stiff problems
        ...
        !> Default: 0.0d0 [-] - no extrapolation (conservative)
        ...
        real(kind=8) :: est_prm=0.0d0 !< [-] estimation parameter for downstream kinetic reactions (0 = no extrapolation)
    contains
        !> \brief Read convergence parameters from `<path><root>_CV_params.dat`
        procedure :: read_CV_params
    end type CV_params_s

    contains
        !> \brief Reads convergence parameters from `<path><root>_CV_params.dat`.
        !> \details Opens the file on a hard-coded unit, scans for the section header
        !>          `CONVERGENCE PARAMETERS`, then reads (in order):
        !>          abs_tol, log_abs_tol, rel_tol, log_rel_tol, est_prm.
        !>          Reading stops at end-of-file or a line containing `end`.
        subroutine read_CV_params(this, path, root)
            ...
            integer(kind=4) :: unit !< Input file unit number (hard-coded)
            integer(kind=4) :: ierr !< I/O error flag
            character(len=256) :: line !< Buffer for section-header lines
            ...
            do
                !> Use formatted read so headers containing spaces (e.g. 'CONVERGENCE PARAMETERS')
                !> are captured whole rather than truncated at the first blank by list-directed I/O.
                read(unit,'(A)',iostat=ierr) line
                if (ierr /= 0) exit
                if (trim(adjustl(line))=='end') exit
                if (trim(adjustl(line))=='CONVERGENCE PARAMETERS') then
