...
!> Variables
    real(kind=8), allocatable :: A(:,:),b(:),c2nc(:),Re(:),rk_new(:),rk_old(:),Sk_nc(:,:)
    integer(kind=4) :: err
!> Pre-process
    allocate(Re(this%solid_chemistry%reactive_zone%speciation_alg%num_eq_reactions)) !> Re=Delta_t*Re (eq reaction ammount per unit volume of water)
!> Process
    !> Linear least squares
    c2nc=this%get_c2nc()
    rk_new=this%get_rk_new()
    ...
    b=matmul(this%solid_chemistry%reactive_zone%stoich_mat(:,this%solid_chemistry%reactive_zone%speciation_alg%num_prim_species+1:&
        ...
        this%solid_chemistry%reactive_zone%speciation_alg%num_prim_species+1:&
        this%solid_chemistry%reactive_zone%speciation_alg%num_var_act_species)),theta*rk_new+(1d0-theta)*rk_old))
    ...
    if (associated(this%gas_chemistry)) then
        this%gas_chemistry%Re(1:this%solid_chemistry%reactive_zone%gas_phase%num_gases_eq_cst_act)=Re(&
            ...
            this%solid_chemistry%reactive_zone%speciation_alg%num_cst_act_species-this%aq_phase%wat_flag)!/Delta_t !> Re_j=Re/Delta_t
