program main_interfaz
    ...
    if (opc_u_tilde.eq.0) then
        ...
        if (ios /= 0) then
            write(*,*) 'Error/EOF leyendo file_u_tilde. Ejecuta en una terminal interactiva o redirige desde fort.5'; call safe_stop(1)
        end if
        ...
        if (tpt_opt.eq.0) then !> compute lambdas
            ...
            if (ios /= 0) then
                write(*,*) 'Error/EOF leyendo mesh_type. Ejecuta en una terminal interactiva o redirige desde fort.5'; call safe_stop(1)
            end if
        ...
        ! read(*,*, iostat=ios) num_comps
        ! if (ios /= 0) then
        !     write(*,*) 'Error/EOF leyendo num_comps. Ejecuta en una terminal interactiva o redirige desde fort.5'; call safe_stop(1)
        ! end if
    ...
    write(*,*) "Procedemos al bucle de mezcla reactiva. Tendras que actualizar el archivo con las concentraciones despues del &
        transporte conservativo en cada iteracion."
