...
subroutine read_target_waters_init(this,root,nsrz,ngrz)
    ...
    integer(kind=4) :: ind_rech,ind_dom,counter_swap,ind,i,j,k,m,nwtype,num_tar_wat,tar_wat_ind,wtype,istype,nstype,nbwtype,bwtype,&
        mix_wat_ind,ngzns,ind_tar_gas,num_tar_wat_rech,aux_col,flag_wat_type,num_tar_wat_bd,ind_bd,unit,first,last,ind_bar,&
        ind_tar_sol
    real(kind=8), allocatable :: c_nc(:),u_init(:,:),c1_init(:),c2_init(:),c2_ig(:),gamma_2aq(:)
    integer(kind=4) :: aux_istype, aux_igzn
    character(len=256) :: label,str,interval!,first_str,last_str
    ...
    do
        ...
        if (label=='end') then
            ...
            if (num_tar_wat_rech>0 .or. num_tar_wat_bd>0) then !> we have recharge or boundary waters
                do 
                    read(unit,*) interval, wtype, ind_tar_sol, ind_tar_gas, flag_wat_type
                    ...
                    else if (ind_tar_gas<0 .or. ind_tar_gas>this%num_target_gases) then
                    ...
                    if (ind_bar>0) then !> we have a range of target waters
                        ...
                        !    error stop "Gas type index out of bounds"
                            ...
                            do i=1,last-first+1
                                this%bd_waters_indices(ind_bd+i)=first+i-1 !> we set the indices of boundary waters
                            end do
                            ind_bd=ind_bd+last-first+1 !> counter boundary waters
                        else if (flag_wat_type==2) then !> recharge water
                            !this%ext_waters_indices(ind_ext)=tar_wat_ind
                            do i=1,last-first+1
                                this%rech_waters_indices(ind_rech+i)=first+i-1 !> we set the indices of recharge waters
                            end do
                            ind_rech=ind_rech+last-first+1 !> counter recharge waters
                        else if (flag_wat_type==1) then !> domain water
                            !this%dom_tar_wat_indices(ind_dom)=tar_wat_ind
                            do i=1,last-first+1
                                this%dom_tar_wat_indices(ind_dom+i)=first+i-1 !> we set the indices of domain waters
                            end do
                            ind_dom=ind_dom+last-first+1 !> counter domain waters
                            flag=.false.
                        !else
                        !    error stop "Water type flag out of bounds"
                        end if
                        do i=first,last
                            call this%read_tar_wat_line(flag,nsrz,ngrz,i,wtype,&
                                ind_tar_sol,ind_tar_gas,aux_istype,aux_igzn)                            
                        end do
                        ...
                        read(interval_trim,*) tar_wat_ind !> target water index
                        ...
                        call this%read_tar_wat_line(flag,nsrz,ngrz,tar_wat_ind,wtype,&
                                ind_tar_sol,ind_tar_gas,aux_istype,aux_igzn)
                            !this%target_waters_init(ind_tar_sol)=init_sol_types(sol_zone) !> we set the initial solid type
                ...
                do! i=1,this%num_target_waters
                    read(unit,*) interval, wtype, ind_tar_sol, ind_tar_gas
                    interval_trim=trim(interval)
                    ind_bar=index(interval_trim,'-')
                    if (wtype<1 .or. wtype>nwtype) then
                        error stop "Water type index out of bounds"
