subroutine solve_flow_EE_Delta_t_homog(this,Time_out,output)
    !> Solves 1D transient flow PDE with homogeneous time step using Euler explicit method 
    
    !> this: transient flow object
    !> Time_out: output time values
    !> output: headentration vs time output
    ...
    real(kind=8), allocatable :: head_old(:),head_new(:)
    ...
    !type is (transport_1D_transient_c)
        ...
        !> BCs
            ...
            else if (this%BCs%labels(1) == 3 .and. this%BCs%labels(2) == 2) then
        ...
        !> Explicit Euler
            open(unit=0,file="head_binary_EE.txt",form="unformatted",access="sequential",status="unknown")
            ...
            !Delta_x=this%spatial_discr%get_Cell_size()
        !> Time integration loop
            do k=1,time_discr%Num_time
                Time=k*time_discr%Delta_t
                write(0) Time, head_old
            !> Spatial discretization
                call this%compute_b_vec_1D(0d0,head_old,head_new)
            !> Mass balance error
                !mass_bal_err=p_mass_bal_err(this,head_old,head_new,time_discr%Delta_t,Delta_x)
