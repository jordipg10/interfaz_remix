!> \file flow_props_heterog_m.f90
!> \brief Heterogeneous flow property classes for groundwater models.
!> \details
!> Defines derived types that store spatially heterogeneous hydraulic
!> properties needed by the flow equation:
!>   - flow_props_heterog_c: specific storage \f$ S_s \f$ [1/m] and
!>     hydraulic conductivity \f$ K \f$ [m/s].
!>   - flow_props_heterog_conf_c: confined-aquifer variant adding
!>     storativity \f$ S \f$ [-] and transmissivity \f$ T \f$ [m^2/s].
!>
!> Property arrays are read from `<root>_flow_props.dat` and may be
!> homogeneous (single value broadcast) or heterogeneous (per-cell).
!>
!> \author Jordi
!> \date Unknown
!> \ingroup transport
!> \see properties_m, flow_transient_m, char_params_flow_m
module flow_props_heterog_m
    use properties_m, only: props_c !> import properties superclass
    ...
    contains
        procedure :: set_props_flow_heterog
        procedure :: read_props=>read_props_flow_heterog
        !procedure :: are_props_homog=>are_flow_props_homog
    end type
    ...
    contains
        procedure :: read_props=>read_props_flow_heterog_conf
    end type
