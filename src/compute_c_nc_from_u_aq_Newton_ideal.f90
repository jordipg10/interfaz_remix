...
!> Process
    do
        ...
        if (niter>this%solid_chemistry%reactive_zone%CV_params%niter_max) then !> max number of iterations reached
            print *, "Residual: ", inf_norm_vec_real(residual)
            print *, "Too many Newton iterations in speciation"
            exit
        end if
        call this%compute_c2nc_from_c1_aq_ideal(c2nc) !> we compute c2nc from c1 (current guess)
        conc_nc(1:n_p)=this%concentrations(this%ind_var_act_species(1:n_p)) !> we set c1 in conc_nc
        conc_nc(n_p+1:n_nc)=c2nc !> we set c2nc in conc_nc
        call this%compute_residual(conc_comp,conc_nc,residual) !> we compute residual
        ...
        if (inf_norm_vec_real(Delta_c1/conc_nc(1:n_p))<this%solid_chemistry%reactive_zone%CV_params%abs_tol**2) then !> we check relative change in c1
            print *, "Relative error: ", inf_norm_vec_real(Delta_c1/conc_nc(1:n_p))
            print *, "Newton speciation not accurate enough"
            exit
        else
            call this%update_conc_aq_prim_species(Delta_c1,zero_flag) !> we update c1
        end if
    end do
end subroutine