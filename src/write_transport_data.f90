!> \file write_transport_data.f90
!> \brief Writes transport data from a 1D reactive transport model.
!> \details
!> Outputs transport properties, spatial discretisation and solution
!> data of an RT_c object to a previously opened file unit.
!>
!> \author Jordi
!> \date Unknown
!> \ingroup io
!> \see RT_m, write_chemistry
...
subroutine write_transport_data(this,unit)
    ...
    use RT_m, only: RT_c, RT_1D_transient_c
    ...
    type is (RT_1D_transient_c)
        ...
        !end if
        !if (this%transport%BCs%labels(1).eqv.1 .and. this%transport%BCs%labels(2).eqv.1) then
        ...
        !else if (this%transport%BCs%labels(1).eqv.2 .and. this%transport%BCs%labels(2).eqv.2) then
        !    write(unit,"(2x,'Boundary conditions:',10x,'Neumann homogeneous',/)")
        !else if (this%transport%BCs%labels(1).eqv.3 .and. this%transport%BCs%labels(2).eqv.2) then
        ...
        !type is (transport_1D_transient_c)
            ...
            do i=1,this%transport%mix_conc_indices%num_cols
                write(unit,"(2x,I5,*(I5))") this%chemistry%num_ext_waters+i, this%transport%mix_conc_indices%cols(i)%col_1
            end do
