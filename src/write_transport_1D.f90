...
subroutine write_transport_1D(this)
    ...
    if (this%spatial_discr%scheme.eq.1) then
        if (this%dimless.eqv..true.) then
            write(file_out,"('transpoRT_cFDS_adim.out')")
        else
            write(file_out,"('transpoRT_cFDS.out')")
        end if
    ...
    if (this%dimless.eqv..false.) then
        if (this%tpt_props_heterog%homog_flag.eqv..true.) then
            write(1,"(2x,'Transport properties:',10x,'Homogeneous',/)")
        else
            write(1,"(2x,'Transport properties:',10x,'Heterogeneous',/)")
        end if
        write(1,"(2x,'Properties:'/)")
        write(1,"(10x,'Dispersion at cell centres:',/)")
        do i=1,n
            write(1,"(20x,ES15.5)") this%tpt_props_heterog%disp_cent(i)
        end do
        write(1,"(/,10x,'Flux cell centres:'/)")
        n_flux=size(this%tpt_props_heterog%flux_cent)
        do i=1,n_flux
            write(1,"(20x,ES15.5)") this%tpt_props_heterog%flux_cent(i)
        end do
