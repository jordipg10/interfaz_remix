...
subroutine read_chem_system_PFLOTRAN(this,path,unit)
    ...
    num_aq_sp,num_sec_aq_sp,exch_cat_ind, n_min_kin, n_gas_kin,tmp_index,kin_react_type,n_r,num_mins,num_species,num_surf_compl,&
    ...
    !type(solid_species_c) :: solid
    !class(solid_species_c), allocatable :: solids(:)
            ...
            do
                ...
                if (exch_cat_ind>0 .and. exch_cat_ind<len(str_trim)-1) then
                    ...
                    if (exch_cat_flag.eqv..true.) then
                        !call this%cat_exch%exch_cats(j)%copy_species(this%aq_phase%aq_species(exch_cat_ind))
                    else
    ...
    if (this%num_redox_kin_reacts>0) then
        call this%read_Monod_DB(path,unit_redox)
    end if
