function mass_balance_error_ADE_trans_Dirichlet_discharge(this,conc_old,conc_new,Delta_t,Delta_x) result(mass_bal_err)
    ...
    inf_term = 2d0*this%tpt_props_heterog%disp_int(1)*(this%BCs%conc_inf-conc_old(1))/Delta_x + &
        this%BCs%flux_inf*this%BCs%conc_inf
    out_term = 2d0*this%tpt_props_heterog%disp_int(n)*(this%BCs%conc_out-conc_old(n))/Delta_x - &
