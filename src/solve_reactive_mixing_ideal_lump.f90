...
subroutine solve_reactive_mixing_ideal_lump(this,root,mixing_ratios,mixing_waters_indices,mixing_waters_indices_dom,&
    ...
    use aqueous_chemistry_m, only: aqueous_chemistry_c, real_array_c, int_array_c, mixing_iter_comp_ideal_lump, & 
        ...
        compute_c_tilde, compute_rk_tilde_expl, water_mixing_iter_EE_kin_ideal_lump, water_mixing_iter_EE_eq_kin_ideal_lump, &
        water_mixing_iter_EI_eq_kin_anal_ideal_opt2_lump, water_mixing_iter_EI_kin_anal_ideal_opt2_lump
...
!> Variables
    ...
    REAL(KIND=8), allocatable :: Rk_Accum(:) !> accumulated reaction amounts
...
!> We select reactive mixing subroutine depending on the nature of the chemical system and the methods to compute Jacobians and integrate in time
    ...
    call mixing_ratios%get_can_vec(this%CV_params%abs_tol,num_can_vec,ind_can_vec,num_non_can_vec,ind_non_can_vec)
    ...
    allocate(Rk_Accum(this%num_target_waters_dom))
    Rk_Accum=0d0
            ...
            do i=1,num_non_can_vec
                ...
                !else
                    ...
                    call compute_c_tilde(target_waters_old(mixing_waters_indices%cols(ind_non_can_vec(i))%col_1(&
                        ...
                        mixing_ratios%cols(ind_non_can_vec(i))%col_1,c_tilde(1:n_nc_aq)) !> we assume mixing ratios are the same for conc and rk (chapuza)
                    !> chapuza
                    ...
                    !    mix_ind+j-1))
                    !mix_waters=target_waters_new(mixing_waters_indices_dom%cols(ind_non_can_vec(i))%col_1(mix_ind:mixing_waters_indices_dom%cols(ind_non_can_vec(i))%dim-2))
                    !call compute_rk_tilde(mix_waters,mixing_ratios_Rk_init%cols(ind_non_can_vec(i))%col_1(mix_ind:),&
                    !    mixing_waters_indices_dom%cols(ind_non_can_vec(i))%col_1(mixing_waters_indices_dom%cols(ind_non_can_vec(i))%dim-1),&
                    !    mixing_waters_indices_dom%cols(ind_non_can_vec(i))%col_1(mixing_waters_indices_dom%cols(ind_non_can_vec(i))%dim),theta,Delta_t,rk_tilde)
                    !!> chapuza
                    !target_waters_new(mixing_waters_indices_dom%cols(ind_non_can_vec(i))%col_1(mix_ind:mixing_waters_indices_dom%cols(ind_non_can_vec(i))%dim-2))=&
                    !    mix_waters
