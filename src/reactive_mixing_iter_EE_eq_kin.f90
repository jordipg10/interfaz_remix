...
!> Arguments
    ...
    !real(kind=8), intent(inout) :: c2v_ig(:) !> initial guess secondary variable activity species concentrations
...
!> Variables
    ...
    real(kind=8), allocatable :: c2v_ig(:)
...
!> Process
    ...
    !> We initialise primary concentrations for speciation
    call initialise_iterative_method(c1_old,c1,mu,c1_ig)
    call this%set_conc_aq_prim_species(c1_ig(1:this%solid_chemistry%reactive_zone%speciation_alg%num_aq_prim_species)) !> we set initial guess aqueous primary species concentrations
    log_gamma_var_act=this%compute_log_act_coeffs_var_act_ideal()
    call this%compute_c2v_from_c1_ideal(c1_ig,log_gamma_var_act,c2v_ig) !> we initialise secondary variable activity concentrations for speciation
    do
        !k=0
        !k_div=0
        !Delta_t_bis=Delta_t
    ...
    !> Speciation for variable activity species concentrations
        ...
        !> We check convergence
            if (.not. CV_flag) then !> NO CV
                ...
                c2v_ig=conc_nc(this%solid_chemistry%reactive_zone%speciation_alg%num_prim_species+1:) !> we update secondary species concentrations
...
!> Post-process
    deallocate(conc_comp_react,c1_ig,c1,c2v_ig,rk_new)
end subroutine