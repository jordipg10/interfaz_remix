...
module chem_out_options_m
    ...
    contains
        ...
        subroutine read_chem_out_options(this,root,target_waters)
            ...
            this%num_vars=0
            unit_local = 52
            open(unit=unit_local,file=root//'_out_options.dat',status='old',action='read')
            do
                ...
                if (label=='end') then
                    ...
                    if (nits<0) then
                        ...
                        if (nits>0) then
                            read(unit_local,*) this%time_steps(2:this%num_time_steps-1)
                        end if
                        this%time_steps(1)=0
                    end if
                    !write(*,*) this%time_steps
                else if (label=='TARGET WATERS') then
                    read(unit_local,*) this%num_target_waters
                    if (this%num_target_waters<0) then
                        ...
                        read(unit_local,*) this%ind_target_waters
                    else
